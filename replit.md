...
- **Home Assistant**: For push notifications, voice integration, and smart trigger webhooks.
  - **Cat Wash Automation** (`/api/webhook/cat-wash`): Water leak detector triggers auto-play of next PDF reading. Opens PDF reader on Fire Tablets and Samsung TV via Fire Stick with `catWashFollow=true&autoplay=true`. The **tablet's browser** handles all TTS playback using OpenAI voice via `<audio>` element — audio outputs through Bluetooth to the Echo speaker. No server-side Alexa Media Player (AMP) calls — AMP causes "Simon Says" errors and uses Alexa's voice instead of OpenAI's. **Auto-continuation**: when all chunks finish, the tablet calls `POST /api/cat-wash/update-progress` with `completed: true` to get the next file URL and auto-navigates. Stop endpoint at `/api/cat-wash/stop` (no auth required). Helper functions: `findNextCatWashFile`, `extractAndChunkPdf`, `openUrlOnFireDevice`, `openUrlOnFireStick`.
  - **Cat Lights Trigger** (`/api/webhook/cat-lights`): When `light.cat_lights` turns ON before Wednesday and the current week's CPPA module hasn't been fully listened to, starts/resumes playback from saved progress. When light turns OFF, stops playback and saves chunk position for later resume. Only applies to CPPA module files, not all readings. Set up a Home Assistant automation to POST `{ "state": "on" }` or `{ "state": "off" }` to this webhook on state changes.
  - **TTS Architecture**: Server extracts PDF text, chunks it (~1500 chars), and serves chunk data. The tablet's PDF reader generates OpenAI TTS audio for each chunk via `/api/tts`, plays it through browser `<audio>` element (Bluetooth → Echo), and highlights words in sync. No AMP/Alexa voice involved.
  ...
  - **Resume Logic**: Cat-wash webhook resets to chunk 0 on each trigger. Cat-lights webhook resumes from saved `lastChunkIndex`. Progress is saved by the tablet via `POST /api/cat-wash/update-progress`.
