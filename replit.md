...
- **Home Assistant**: For push notifications, voice integration, and smart trigger webhooks.
  ...
  - **TTS Architecture**: Server extracts PDF text, chunks it (~1500 chars), and serves chunk data. The tablet's PDF reader generates OpenAI TTS audio for each chunk via `/api/tts`, plays it through browser `<audio>` element (Bluetooth → Echo), and highlights words in sync. No AMP/Alexa voice involved. **Server-side TTS fallback**: If no tablet progress update within 30 seconds of a trigger, server automatically generates TTS chunks via OpenAI and plays them directly to the Echo speaker group (`media_player.cat_wash_2`) via `media_player.play_media` — no tablet, Silk, or Bluetooth needed. Fallback is active in both cat-wash and cat-lights webhooks.
