...
- **Home Assistant**: For push notifications, voice integration, and smart trigger webhooks (e.g., play urgent PDF via Alexa routines).
  - **Cat Wash Automation** (`/api/webhook/cat-wash`): Water leak detector triggers auto-play of next PDF reading. TTS uses `notify/alexa_media` with SSML (`<speak><prosody rate="90%">TEXT</prosody></speak>`) and `data: { type: "tts" }` to play on Echo speakers (avoids Amazon's blocked `media_player/play_media` + `music` streaming). Opens PDF reader on Fire Tablets and Samsung TV via Fire Stick using `command_launch_app` with Silk browser package (`com.amazon.cloud9`) + `android.intent.action.VIEW` intent (avoids Browser Mod conflicts from `command_webview`). `catWashFollow=true` mode for synced text highlighting. **Auto-continuation**: when a PDF finishes, announces completion and next file name/course/folder, then automatically begins playing the next PDF in the priority list. When all files for the week are done, announces completion and switches to radio. Progress endpoint at `/api/cat-wash/progress`. Stop endpoint at `/api/cat-wash/stop`. Helper functions: `findNextCatWashFile`, `extractAndChunkPdf`, `playCatWashFile`, `sendCatWashTTSToEcho`, `openUrlOnFireDevice`.
  - **TTS Approach**: `sendCatWashTTSToEcho` first generates OpenAI TTS audio, then embeds it as SSML `<audio src="URL">` in `notify/alexa_media` for natural voice. Falls back to Alexa's built-in TTS at 75% speed with academic-style SSML formatting (breaks after headers/paragraphs). Chunks are 1500 chars max for manageable TTS segments. Duration is estimated from word count (~145 wpm for OpenAI, ~120 wpm for Alexa fallback).
  - **Device Opening**: Tablets use `command_activity` → `command_broadcast_intent` → `command_webview` fallback chain. Fire Sticks use `androidtv/adb_command` to launch Silk with URL intent → `media_player/play_media` with `url` type fallback. Helper: `openUrlOnFireStick`.
  - **Resume Logic**: Each webhook trigger resets progress to chunk 0 (clears stale `lastChunkIndex`/`checkedChunks` from failed sessions). Progress tracks during active playback only.
