...
export interface IStorage {
  ...
  getCourseWeekMappings(courseCode: string, semesterSettingsId: number): Promise<CourseWeekMapping[]>;
  getAllCourseWeekMappings(semesterSettingsId: number): Promise<CourseWeekMapping[]>;
  upsertCourseWeekMapping(data: InsertCourseWeekMapping): Promise<CourseWeekMapping>;
  ...
  getPdfAnnotations(fileId: number): Promise<PdfAnnotation[]>;
  createPdfAnnotation(annotation: InsertPdfAnnotation): Promise<PdfAnnotation>;
  deletePdfAnnotation(id: number): Promise<void>;
  createSharedLibraryToken(semesterKey?: string): Promise<SharedLibraryToken>;
...
export class DatabaseStorage implements IStorage {
  async getTasks(filters?: { weekNumber?: number; type?: string; showCompleted?: boolean }): Promise<Task[]> {
    ...
    if (filters?.weekNumber) {
      // Include tasks from previous, selected, and following week (for calendar visibility)
      conditions.push(or(
  ...
  async updateFile(id: number, updates: { displayName?: string; folder?: string | null; listened?: boolean; lastChunkIndex?: number; totalChunks?: number; checkedChunks?: string; ttsAudioUrl?: string; ttsGeneratedAt?: Date; objectPath?: string; extractedText?: string; preparedAudioPaths?: string; preparedAt?: Date; captionsStatus?: string | null; captionsError?: string | null; captionsGeneratedAt?: Date | null }): Promise<FileRecord> {
    ...
    if (updates.checkedChunks !== undefined) {
      setData.checkedChunks = updates.checkedChunks;
    }
    if (updates.ttsAudioUrl !== undefined) {
      setData.ttsAudioUrl = updates.ttsAudioUrl;
    }
  ...
  async getCourseWeekMappings(courseCode: string, semesterSettingsId: number): Promise<CourseWeekMapping[]> {
    return await db.select().from(courseWeekMappings).where(
      and(eq(courseWeekMappings.courseCode, courseCode), eq(courseWeekMappings.semesterSettingsId, semesterSettingsId))
    );
  }

  async getAllCourseWeekMappings(semesterSettingsId: number): Promise<CourseWeekMapping[]> {
    return await db.select().from(courseWeekMappings).where(
      eq(courseWeekMappings.semesterSettingsId, semesterSettingsId)
    );
  }

  async upsertCourseWeekMapping(data: InsertCourseWeekMapping): Promise<CourseWeekMapping> {
    const existing = await db.select().from(courseWeekMappings).where(
      and(
        eq(courseWeekMappings.courseCode, data.courseCode),
        eq(courseWeekMappings.semesterSettingsId, data.semesterSettingsId),
        eq(courseWeekMappings.weekNumber, data.weekNumber)
      )
    );
    if (existing.length > 0) {
      const [updated] = await db.update(courseWeekMappings)
        .set({ confirmed: data.confirmed, courseWeekLabel: data.courseWeekLabel, notes: data.notes })
        .where(eq(courseWeekMappings.id, existing[0].id))
        .returning();
      return updated;
    }
    const [created] = await db.insert(courseWeekMappings).values(data).returning();
    return created;
  }
  ...
  async getAnnouncements(): Promise<Announcement[]> {
    return db.select().from(announcements).orderBy(announcements.sortOrder, announcements.receivedAt);
  }
  ...
  async getSavedEmailSearches(): Promise<SavedEmailSearch[]> {
    return await db.select().from(savedEmailSearches).orderBy(desc(savedEmailSearches.createdAt));
  }

  async createSavedEmailSearch(data: InsertSavedEmailSearch): Promise<SavedEmailSearch> {
    const [created] = await db.insert(savedEmailSearches).values(data).returning();
    return created;
  }
  ...
  async getPdfAnnotations(fileId: number): Promise<PdfAnnotation[]> {
    return await db.select().from(pdfAnnotations).where(eq(pdfAnnotations.fileId, fileId)).orderBy(pdfAnnotations.page);
  }

  async createPdfAnnotation(annotation: InsertPdfAnnotation): Promise<PdfAnnotation> {
    const [created] = await db.insert(pdfAnnotations).values(annotation).returning();
    return created;
  }

  async deletePdfAnnotation(id: number): Promise<void> {
    await db.delete(pdfAnnotations).where(eq(pdfAnnotations.id, id));
  }

  async createSharedLibraryToken(semesterKey?: string): Promise<SharedLibraryToken> {
    const token = Math.random().toString(36).substring(2, 10) + Math.random().toString(36).substring(2, 10);
    const [created] = await db.insert(sharedLibraryTokens).values({ token, semesterKey: semesterKey || null }).returning();
    return created;
  }

  async getSharedLibraryToken(token: string): Promise<SharedLibraryToken | null> {
    const [result] = await db.select().from(sharedLibraryTokens).where(and(eq(sharedLibraryTokens.token, token), eq(sharedLibraryTokens.isActive, true)));
    return result || null;
  }

  async deactivateSharedLibraryToken(id: number): Promise<void> {
    await db.update(sharedLibraryTokens).set({ isActive: false }).where(eq(sharedLibraryTokens.id, id));
  }
