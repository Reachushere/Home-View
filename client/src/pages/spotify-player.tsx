...
const ROOM_HOTSPOTS: { room: string; x: number; y: number; w: number; h: number; entityId: string; groupEntityId: string; deviceType: string; icon: string; nightImg?: string; labelOffsetX?: number; labelOffsetY?: number; volumeOffsetX?: number; volumeOffsetY?: number; hideLabel?: boolean }[] = [
  ...
  { room: "Queen Bedroom", x: 2, y: 38, w: 18, h: 33, entityId: "media_player.queen_bedroom", groupEntityId: "media_player.queen_bedroom_media_group", deviceType: "echo", icon: "bed", nightImg: queenNight, labelOffsetX: 17, labelOffsetY: -95, volumeOffsetX: 17, volumeOffsetY: -45 },
  { room: "Pug Washroom", x: 2, y: 5, w: 16, h: 32, entityId: "media_player.echo_show_pug_am", groupEntityId: "media_player.pug_media_group", deviceType: "echo_show", icon: "bath", nightImg: pugNight, labelOffsetX: 30, labelOffsetY: -35, volumeOffsetX: 30, volumeOffsetY: 15 },
  { room: "Hallway", x: 19, y: 5, w: 16, h: 32, entityId: "media_player.hallway_2", groupEntityId: "media_player.hallway_media_group", deviceType: "echo", icon: "hallway", nightImg: hallwayNight, labelOffsetX: 45, labelOffsetY: -235, volumeOffsetX: 45, volumeOffsetY: -190 },
  { room: "Kitchen", x: 36, y: 5, w: 28, h: 45, entityId: "media_player.kitchen_lr", groupEntityId: "media_player.kitchen_media_group", deviceType: "echo", icon: "kitchen", nightImg: kitchenNight, labelOffsetX: -85, labelOffsetY: -230, volumeOffsetX: -85, volumeOffsetY: -190 },
  { room: "Living Room", x: 36, y: 52, w: 28, h: 45, entityId: "media_player.kitchen_lr", groupEntityId: "media_player.living_room_media_group", deviceType: "echo", icon: "sofa", nightImg: livingRoomNight, labelOffsetX: 60, labelOffsetY: -85, volumeOffsetX: 60, volumeOffsetY: -40 },
  { room: "King Bedroom", x: 65, y: 30, w: 33, h: 50, entityId: "media_player.king_bedroom", groupEntityId: "media_player.king_bedroom_media_group", deviceType: "echo", icon: "crown", nightImg: kingNight, labelOffsetX: 70, labelOffsetY: -354, volumeOffsetX: 70, volumeOffsetY: -304 },
  { room: "Cat Washroom", x: 84, y: 3, w: 14, h: 26, entityId: "media_player.cat_speakers", groupEntityId: "media_player.cat_washroom_media_group", deviceType: "echo", icon: "bath", nightImg: catNight, labelOffsetX: -395, labelOffsetY: -155, volumeOffsetX: -395, volumeOffsetY: -115 },
  { room: "Closet", x: 65, y: 3, w: 18, h: 26, entityId: "media_player.echo_closet_am", groupEntityId: "media_player.closet_media_group", deviceType: "echo", icon: "closet", nightImg: closetNight, labelOffsetX: 65, labelOffsetY: -109, volumeOffsetX: 65, volumeOffsetY: -69 },
...
export default function SpotifyPlayerPage() {
  ...
  const createGhost = (label: string, imageUrl?: string) => {
  ...
  useEffect(() => {
    const onMove = (cx: number, cy: number, evt: Event) => {
      ...
      if (!cd.movedEnough && dist > 5) {
        ...
        cd.ghostEl = createGhost(cd.data?.name || cd.data?.room || "?", cd.imageUrl);
              ...
              <div className="overflow-y-auto flex-1" style={{ scrollbarWidth: "none", maxHeight: 'calc(100% - 24px)' }}>
                <div className="grid grid-cols-2 gap-1.5">
                  {profile.artists.map((artist, i) => {
                    ...
                    return (
                    <div key={artist.name}
                      {...handleDragItemDown("artist", artist, artistImages[artist.name] || artist.image)}
                      onClick={() => {
