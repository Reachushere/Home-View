                    ...
                    <div className="flex items-center gap-0.5 px-1.5 py-0.5 rounded-md" style={{ background: 'rgba(255,255,255,0.15)', border: '1px solid rgba(255,255,255,0.4)' }}>
                      <select className="h-4 text-[10px] bg-transparent border-none text-white rounded px-0.5 outline-none cursor-pointer" style={{ WebkitAppearance: 'none', appearance: 'none', paddingRight: '12px' }} value={courseRankA ?? 0} onChange={(e) => {
                        ...
                        {[1, 2].map(n => {
                          ...
                          return <option key={n} value={n} className="bg-gray-800" disabled={taken} style={taken ? { color: '#555' } : {}}>{`1${n === 1 ? 'A' : 'B'}`}</option>;
                        })}
                      </select>
                    ...
                    <div className="flex items-center gap-0.5 px-1.5 py-0.5 rounded-md" style={{ background: 'rgba(255,255,255,0.15)', border: '1px solid rgba(255,255,255,0.4)' }}>
                      <select className="h-4 text-[10px] bg-transparent border-none text-white rounded px-0.5 outline-none cursor-pointer" style={{ WebkitAppearance: 'none', appearance: 'none', paddingRight: '12px' }} value={courseRankB ?? 0} onChange={(e) => {
                        ...
                        {[1, 2].map(n => {
                          ...
                          return <option key={n} value={n} className="bg-gray-800" disabled={taken} style={taken ? { color: '#555' } : {}}>{`2${n === 1 ? 'A' : 'B'}`}</option>;
                        })}
                      </select>
                      <ChevronDown className="w-3 h-3 text-white/60 -ml-4 pointer-events-none" />
                    </div>
              ...
              return (
                <div className="flex items-end px-1.5 py-1 text-[8px] font-bold text-white" style={{ margin: '0 4px', marginTop: '10px', letterSpacing: '0' }}>
                  ...
                  <div className="flex-shrink-0 text-white" style={{ position: 'relative', width: '162px', height: '20px', left: isEditingInfo ? '-31px' : '-22px' }}>
                    <span className={`w-[33px] text-center leading-tight ${hdrCls('score')}`} onClick={() => toggleSort('score')} style={{ display: 'inline-flex', justifyContent: 'center', position: 'absolute', left: '-5px', bottom: 0 }} data-testid="sort-score">
                      Score<SortIcon field="score" />
                    </span>
                    <span className={`w-[33px] text-center leading-tight ${hdrCls('total')}`} onClick={() => toggleSort('total')} style={{ display: 'inline-flex', justifyContent: 'center', position: 'absolute', left: '38px', bottom: 0 }} data-testid="sort-total">
                      Total<SortIcon field="total" />
                    </span>
                    <span className={`w-[33px] text-center leading-tight ${hdrCls('weight')}`} onClick={() => toggleSort('weight')} style={{ display: 'inline-flex', justifyContent: 'center', position: 'absolute', left: '82px', bottom: 0 }} data-testid="sort-weight">
                      Weight<SortIcon field="weight" />
                    </span>
                    <span className={`w-[33px] text-center leading-tight ${hdrCls('percent')}`} onClick={() => toggleSort('percent')} style={{ display: 'inline-flex', justifyContent: 'center', position: 'absolute', left: '128px', bottom: 0 }} data-testid="sort-percent">
              ...
              <>
                <div className="flex items-center px-1.5 py-1.5 text-[11px] font-medium text-white uppercase" style={{ margin: '12px 4px 0 4px' }} data-testid="ungraded-section-header">
                  ...
                  <div className="flex items-center" style={{ gap: '6px', marginRight: '24px' }}>
                    <span className="text-[8px] text-white whitespace-nowrap" style={{ display: 'inline-block', width: '40px', textAlign: 'left', letterSpacing: '0.5px' }}>graded</span>
