                ...
                <div className="mt-1 rounded" style={{ border: '1px solid rgba(255,255,255,0.15)', background: 'rgba(0,0,0,0.4)', padding: '6px', overflow: 'visible' }}>
                  ...
                  <div className="relative rounded cursor-crosshair" style={{ height: '92px', touchAction: 'none', overflow: 'visible' }} data-testid={`color-area-${activeGradientStop}`}
                    ...
                    {(() => {
                      ...
                      return (
                        <div style={{ position: 'absolute', left: `${pos.x * 100}%`, top: `${pos.y * 100}%`, transform: 'translate(-50%, -50%)', width: '14px', height: '14px', borderRadius: '50%', border: '2px solid white', boxShadow: '0 0 3px rgba(0,0,0,0.5), inset 0 0 1px rgba(0,0,0,0.3)', pointerEvents: 'none', backgroundColor: getActiveColor() }} />
                      );
                    })()}
                  </div>
          ...
          {activeSwatchPicker && (() => {
            ...
            return (
              <div className="rounded" style={{ border: '1px solid rgba(255,255,255,0.15)', background: 'rgba(0,0,0,0.4)', padding: '6px' }}>
                ...
                <div className="relative rounded cursor-crosshair" style={{ height: '92px', touchAction: 'none' }} data-testid={`swatch-color-area-${activeSwatchPicker}`}
                  onPointerDown={(e) => {
                    e.preventDefault();
                    e.stopPropagation();
                    const el = e.currentTarget as HTMLElement;
                  ...
                  <div style={{ position: 'absolute', inset: 0, borderRadius: '3px', background: `linear-gradient(to right, white, hsl(${hexToHue(hexVal)}, 100%, 50%))`, pointerEvents: 'none' }} />
                  <div style={{ position: 'absolute', inset: 0, borderRadius: '3px', background: 'linear-gradient(to bottom, transparent, black)', pointerEvents: 'none' }} />
                  {(() => {
                    const pos = hexToSvPos(hexVal);
                    return (
                      <div style={{ position: 'absolute', left: `${pos.x * 100}%`, top: `${pos.y * 100}%`, transform: 'translate(-50%, -50%)', width: '14px', height: '14px', borderRadius: '50%', border: '2px solid white', boxShadow: '0 0 3px rgba(0,0,0,0.5), inset 0 0 1px rgba(0,0,0,0.3)', pointerEvents: 'none', backgroundColor: hexVal }} />
                    );
                  })()}
                </div>
