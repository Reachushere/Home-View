            ...
            return (
              <div className="rounded" style={{ border: '1px solid rgba(255,255,255,0.15)', background: 'rgba(0,0,0,0.4)', padding: '6px' }}>
                ...
                <div className="relative rounded cursor-crosshair" style={{ height: '92px', touchAction: 'none' }} data-testid={`swatch-color-area-${activeSwatchPicker}`}
                  ...
                  <div style={{ position: 'absolute', top: '6px', left: '6px', right: '6px', bottom: '6px', borderRadius: '3px', background: `linear-gradient(to right, white, hsl(${hexToHue(hexVal)}, 100%, 50%))` }} />
                  <div style={{ position: 'absolute', top: '6px', left: '6px', right: '6px', bottom: '6px', borderRadius: '3px', background: 'linear-gradient(to bottom, transparent, black)' }} />
                  {(() => {
                    const pos = hexToSvPos(hexVal);
                    return (
                      <div style={{ position: 'absolute', left: `calc(6px + ${pos.x} * (100% - 12px))`, top: `calc(6px + ${pos.y} * (100% - 12px))`, transform: 'translate(-50%, -50%)', width: '12px', height: '12px', borderRadius: '50%', border: '2px solid white', boxShadow: '0 0 3px rgba(0,0,0,0.5), inset 0 0 1px rgba(0,0,0,0.3)', pointerEvents: 'none', backgroundColor: hexVal }} />
                    );
                  })()}
                </div>
